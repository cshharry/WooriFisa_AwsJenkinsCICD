...
## 🐋 Docker	Jenkins를 컨테이너화하여 관리 및 실행 환경을 표준화
## 🧩 Jenkins	CI/CD 파이프라인을 자동화하고 GitHub와 연동하여 빌드를 관리
## ☁️ AWS S3	빌드된 JAR 파일을 저장하고 배포를 위한 버킷으로 사용
## 🐘 AWS RDS	MySQL 데이터베이스를 사용하여 Spring Boot 애플리케이션과 연동
## 🧑‍💻 ngrok	로컬 Jenkins 서버를 외부에서 접근할 수 있도록 터널링
## 🌱 Spring Boot	Java 기반 애플리케이션 프레임워크로, API 및 웹 애플리케이션 구축
## 🐧 Linux	EC2 인스턴스에서 애플리케이션을 호스팅하고 관리하기 위한 OS
## 📡 Slack	빌드 및 배포 상태를 알리기 위한 알림 시스템으로 사용
## 📜 GitHub	소스 코드 버전 관리 및 Jenkins와 연동하여 자동 빌드 트리거
## 💾 AWS CLI	Jenkins에서 AWS S3에 파일을 업로드하고 관리하는 도구
